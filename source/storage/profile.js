...
function normalizeProfile(raw) {
  ...
  const sessions = Array.isArray(src.sessions)
    ...
    : [];
  // Явный whitelist не даёт старым/случайным полям из localStorage
  // бесконечно путешествовать между версиями профиля.
  return {
...
/*
 * Локальное хранилище остаётся только прототипом до authoritative server.
 * В v52 профиль уже изолирован по аккаунту, а незавершённая локальная
 * сессия не может навсегда «съесть» внесённый капитал: после reload точное
 * состояние браузерного движка восстановить нельзя, поэтому возвращается
 * именно первоначальный взнос и activeSession очищается.
 */
const profileStore = {
  async load(account) {
    ...
    try {
      ...
      if (found) profile = normalizeProfile(JSON.parse(found));

      // Одноразовая миграция v51: старый глобальный профиль принадлежит
      // аккаунту, который был активен в момент первого запуска v52.
      if (!profile) {
