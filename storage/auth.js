...
const authStore = {
    async current() {
        ...
        if (session?.email)
            return publicAccount(session.email, session.at);
        const persistent = readJson(localStorage, ACCOUNT_KEY, null);
        if (persistent?.email)
            return publicAccount(persistent.email, persistent.at);
    ...
    async signUp(email, password, remember = true) {
        ...
        if (!EMAIL_RE.test(id))
            return { ok: false, reason: "неверный email" };
        if (String(password || "").length < 6)
            return { ok: false, reason: "пароль слишком короткий" };
        const db = loadAccountDb();
        if (db[id])
            return { ok: false, reason: "аккаунт уже существует" };
        const salt = makeSalt();
        const createdAt = Date.now();
        db[id] = { salt, hash: await credentialHash(id, password, salt), createdAt };
        if (!saveAccountDb(db))
            return { ok: false, reason: "не удалось сохранить аккаунт" };
