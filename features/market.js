...
const EYES_BAND = 0.002;
const EYES_GAP = 0.0035;
const EYES_HISTORY = 40;
const EYES_HISTORY_MS = 180000;
class EyesLayer {
    constructor() {
        this.state = new Map();
        this.history = [];
    ...
    update(market, executed, now) {
        ...
        for (const pl of market.players) {
            ...
            if (pl.stopLoss !== null)
                raw.push({ type: "STOP", side, price: pl.stopLoss, volume });
            if (pl.takeProfit !== null)
                raw.push({ type: "TAKE", side, price: pl.takeProfit, volume });
        }
        ...
        for (const [k, list] of groups) {
            ...
            for (const o of list) {
                if (cur && o.price - cur.max > P * EYES_GAP)
                    flush();
                if (!cur)
                    cur = { min: o.price, max: o.price, volume: 0, n: 0 };
                cur.max = o.price;
                cur.volume += o.volume;
                cur.n++;
            }
...
class LegacyRoom {
    constructor({ startingCapital = 100, seed = 1, devMode = true, playerCount, leverage = 1, eyes = false, durationTicks = null, warmupTicks = CONFIG.market.warmupTicks } = {}) {
    ...
    advance(n) {
        for (let k = 0; k < n && !this._room.halted; k++)
            this.step();
        return this;
    }
    step() {
        if (this.paused || this._room.halted)
            return null;
        ...
        if (this._priceHistory.length > 6000)
    ...
    snapshotFor(viewerId) {
        ...
        if (fresh)
            this._cache = { tick, participants: base.participants,
    ...
    _context(base) {
        ...
        for (const p of parts) {
            ...
            if (p.position.side === "long")
                L += p.position.invested;
            else
                S += p.position.invested;
        }
    ...
    _aggregate(participants) {
        ...
        for (const p of participants) {
            ...
            if (p.position) {
                if (p.position.side === "long") {
                    longExposure += p.closeValue;
                    longPlayers++;
                }
                else {
                    shortExposure += p.closeValue;
                    shortPlayers++;
                }
            }
        }
...
const FREE_MARKET = {
    ...
    rosterSample: 120,
    historyLimit: 36000,
    historyTrimChunk: 2000,
    lifecycleEvery: 50,
    lifecycleMaxPerPass: 8,
    retireMinTicks: 9000,
    retireMaxTicks: 54000,
    distressFraction: 0.035,
};
...
function freeMarketLifecycle(m) {
    if (!m.freeMarket || m.tick % FREE_MARKET.lifecycleEvery !== 0)
        return;
    ...
    while (seen < FREE_MARKET.botCount && rotated < FREE_MARKET.lifecycleMaxPerPass) {
        ...
        if (!n.retireRequested && (distressed || expired))
            n.retireRequested = true;
        if (!n.retireRequested || p.u !== 0)
            continue;
        resetFreeBot(m, p);
        rotated++;
        curveDirty = true;
...
class FreeMarketLegacyRoom {
    constructor({ startingCapital = 0, seed = 7001, devMode = false, attachHuman = true } = {}) {
        const requestedHumanCapital = parseMoneyInput(startingCapital);
        const count = FREE_MARKET.botCount + 1;
        ...
        m.freeTurnover = 0;
        for (let i = 1; i < count; i++)
            applyFreePersonality(m.players[i], r, 0, 0);
    ...
    attachHuman(capital) {
        ...
        if (this._humanAttached)
            return this.snapshotFor(0);
        p.cash = amount;
        p.u = 0;
        p.startingCapital = amount;
        p.entryPrice = null;
        p.invested = 0;
        p.basis = 0;
        p.realizedPnL = 0;
    ...
    detachHuman() {
        if (!this._humanAttached)
            return { equity: 0, snapshot: this.snapshotFor(0) };
        const m = this._room.market;
        const p = m.players[0];
        if (p.u !== 0) {
    ...
    _freeAggregate(viewerId) {
        ...
        for (let i = 0; i < m.players.length; i++) {
            ...
            if (i !== viewerId && ret > meReturn)
                rank++;
            if (p.u > 0) {
                longPlayers++;
                longExposure += m.settlement(i);
            }
            else if (p.u < 0) {
                shortPlayers++;
                shortExposure += m.settlement(i);
            }
            if (i > 0) {
                if (p.u !== 0)
                    activeBots++;
...
class LocalTransport {
    constructor({ startingCapital, seed, devMode = true, leverage = 1, eyes = false, durationTicks = null, warmupTicks = CONFIG.market.warmupTicks, playerCount = null } = {}) {
        this.leverage = leverage;
        this.eyes = eyes;
        this.room = new Room({ startingCapital, seed, devMode, leverage, eyes,
            durationTicks, warmupTicks, playerCount });
        this.playerId = this.room.join(null, "ВЫ");
        this.timer = null;
        this.speed = 1;
    }
    start(onSnapshot) {
...
class FreeMarketHub {
    ...
    maybeStop() {
        if (!this.room._humanAttached && this.subscribers.size === 0)
            this.stop();
    }
    _pump() {
...
function getFreeMarketHub() {
    if (!FREE_MARKET_HUB)
        FREE_MARKET_HUB = new FreeMarketHub();
...
class FreeMarketTransport {
    constructor({ startingCapital } = {}) {
        ...
        if (!snap)
            throw new Error("invalid free market starting capital");
    }
    start(onSnapshot) {
        this.stop();
    ...
    async send(command) {
        if (this.left)
            return { ok: false, reason: "вы уже вышли из свободного рынка" };
        return this.hub.send(command);
    }
    snapshot() { return this.finalSnapshot || this.hub.snapshot(); }
    leave() {
        if (this.left)
            return { equity: this.finalSnapshot?.you?.equity || 0,
...
function buildCandles(points, bucketMs, maxCandles) {
    ...
    let start = n - 1;
    while (start > 0 && points[start - 1].t >= earliest)
        start--;
    const candles = [];
    let current = null;
